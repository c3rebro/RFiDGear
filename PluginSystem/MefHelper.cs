...
public sealed class MefHelper : IDisposable
{
    ...
    private MefHelper()
    {
        ...
        sPathInitial = System.Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData);

        System.Reflection.Assembly ass = System.Reflection.Assembly.GetEntryAssembly();

        _ExtensionsPath = System.IO.Path.Combine(sPathInitial, ass.GetName().Name);

        try
        {
            if (System.IO.Directory.Exists(_ExtensionsPath) == false)
                System.IO.Directory.CreateDirectory(_ExtensionsPath);
        }
        catch (Exception ex)
        {
        }

        _ExtensionsPath = System.IO.Path.Combine(_ExtensionsPath, "Extensions");
        ...
        #if (DEBUG)
        _ExtensionsPath = AppDomain.CurrentDomain.BaseDirectory;
        #endif
    }
